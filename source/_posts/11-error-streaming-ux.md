---
title: 错误处理与流式UX：分层重试 + EventStream + reducer
description: >-
  外置分层重试哲学、三层重试（传输/业务/语义）、错误不持久化、AgentEvent联合类型、EventStream → UI reducer
  模式、流式渲染/回合中断/多端同步、resync协议、turn_busy、SessionRef、三种错误处理路线对比
tags:
  - 错误处理
  - 流式传输
  - 事件流
...
Agent系统的"可靠性"和"UX"看似两个话题，在aptbot里其实是同一个——都是关于"事件如何从agent流到用户、出错时如何处理"。这篇文章把这两条线索拧在一起，看aptbot的错误处理哲学、事件流抽象、UI渲染模式如何协作。

## 一、概念：错误处理与流式UX是同一个问题

在传统应用中，错误处理和用户体验是两个独立的关注点。后端负责"不出错"，前端负责"体验好"。但在agent系统中，这两者高度耦合——agent的输出本身就是流式的（LLM逐token返回、工具调用与结果交错出现），错误可能在任何时刻发生在任何一层（网络错误、LLM错误、工具执行错误），而用户需要实时看到"当前agent在做什么"和"出错了怎么办"。

换个角度看：**agent的用户体验，本质上就是错误处理**。因为agent的运行过程充满了不确定性——LLM可能输出非法JSON、工具可能超时、Provider可能不可用——每一次不确定性都是一次潜在的"错误"。一个好的agent系统不是"不出错"，而是"出错时让用户知道发生了什么、系统正在做什么、接下来会怎样"。这才是agent UX的核心。
...
事件驱动模式的一个变体是事件流 + reducer模式——不仅决策用事件驱动，UI渲染也用事件驱动。核心循环产生类型化的事件流，经过reducer折叠成UI状态，UI根据状态渲染。错误事件、正常事件、状态变化事件都在同一个事件流中流动。

这个模式让"错误UX"和"正常UX"在同一个框架下处理——错误不是"特殊路径"，只是事件流中的一种事件类型。reducer处理它，UI渲染它，用户看到它。
...
UI不是直接处理事件流，而是通过reducer模式：

- **EventStream**：事件的有序序列，从agent流到UI
- **reducer**：纯函数 `(state, event) => newState`，把事件序列"折叠"成UI状态
- **UI**：根据state渲染，state变化时自动重渲染

![错误处理与流式事件流](/images/11-error-streaming-ux/error-streaming.png)

aptbot有两个UI：CLI（使用Ink）和WebUI（使用Lit）。它们使用**同一个 `coreReducer`**——reducer是纯函数，与UI框架无关。差异只在"如何把state渲染成像素"——Ink渲染成终端字符、Lit渲染成DOM元素。

reducer共享的好处：

1. **一致性**：CLI和WebUI显示同样的agent状态，不会出现"CLI看到工具调用但WebUI看不到"的情况
2. **可测试**：reducer是纯函数，单元测试不需要拉起UI框架
3. **可演化**：未来添加新UI（如mobile app），只需要复用reducer，只写渲染层
...
reducer模式让三个看似复杂的UX行为变成了"事件流的自然消费"：

**流式渲染**：token event逐个到达，reducer把它们append到state的"当前assistant message"字段，UI检测到state变化就渲染新token。不需要特殊的"流式逻辑"，就是reducer处理token event的自然结果。

**回合中断**：用户点击"停止"按钮，发送abort信号给agent loop，loop停止工具执行与LLM调用，发送turn_aborted event。reducer收到turn_aborted，把当前message标记为"已中断"，UI显示中断标记。中断不是"特殊路径"，是事件流中的一个event。

**多端同步**：agent事件发送到bus，bus分发给所有绑定该session的channel。每个channel的UI各自运行reducer，state各自演化但保持同步——因为它们消费的是同一份事件流。多端同步不需要"特殊的同步逻辑"，是事件流的天然结果。

这就是reducer模式的核心价值——把"复杂的UX行为"还原成"事件流 + 纯函数"，复杂度从UI层转移到事件设计层，UI层因此变薄。
...
- agent给客户端发送turn_busy event，告诉用户"我现在忙，你的消息已排队"
- 当前turn结束后，agent自动处理队列中的下一条消息

这让UX更清晰——用户知道"我的消息被收到了，但agent正在忙，稍后处理"，而不是"我发了消息但agent没有反应"的迷茫。turn_busy是事件流中的又一个event，reducer处理它并显示"忙"状态。

### 4.9 SessionRef可变引用（运行中切换session不重启loop）
...
错误处理与流式UX在aptbot中被统一为同一个问题——"事件如何流动、出错时如何处理"。外置分层重试哲学让决策集中，传输/业务/语义三层重试各管各的，错误不持久化防止400 poisoning。AgentEvent联合类型 + EventStream + reducer让UI层变薄，流式渲染、回合中断、多端同步都是事件流的自然消费。resync协议支撑无感重连，turn_busy让队列反馈清晰，SessionRef让运行中切换session不需要重启loop。

对比方案A（内联处理）和方案B（事件emit + 上层监听），aptbot选择方案C（类型化EventStream + reducer）的原因不只是"更结构化的错误处理"——更是因为在多端UI的场景下，reducer共享提供了成本最低的一致性保证。而"错误即事件"的思维模型，让agent系统中所有的不确定性都在同一个框架下处理，开发者不需要区分"正常路径"和"异常路径"——一切都是事件流。
